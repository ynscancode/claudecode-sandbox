...
function emptyNormalForm() {
  return { date: todayStr(), account_id: String(ACCOUNTS.SPENDING), direction: 'out', category: OUTGOING_CATEGORIES[0], amount: '', comment: '' }
}
...
export default function TransactionModal({ initialMode = 'normal', onClose, onCreateTransaction, onCreateTransfer }) {
  ...
  const [normalForm, setNormalForm] = useState(emptyNormalForm)
  const [transferForm, setTransferForm] = useState(emptyTransferForm)
  const [error, setError] = useState(null)
  const [submitting, setSubmitting] = useState(false)

  const normalCategories = normalForm.direction === 'out' ? OUTGOING_CATEGORIES : INCOMING_CATEGORIES
      ...
      <div className="modal-panel" onClick={(e) => e.stopPropagation()}>
        ...
        {mode === 'normal' ? (
          <div className="modal-form-grid">
            ...
            <label className="form-field">
              Category
              <select value={normalForm.category} onChange={(e) => setNormalField('category', e.target.value)}>
